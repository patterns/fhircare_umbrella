defmodule Fhirshape.V1.VanillaObservationTest do
  ...
  @rtype "observation"
  @rname "Observation"
  @rpath "/api/v1/observations"
  @invalid_attrs %{resourceType: "INVALID-TYPE"}
  @base_attrs %{
    resourceType: @rname,
    context: %{:id => "cd78b515-5a32-4db0-81c6-3ce87904497f", :type => "TEST"}
  }
  ...
  defmodule VanillaServer do
    ...
    @base_resp %{"resourceType" => "Observation"}

    def read(req, _stream) do
      case Ecto.UUID.cast(req.id) do
        :error ->
          # At the moment, non-existent observation UUID causes database error which is how the gRPC service throws the exception.
    ...
    def create(req, _stream) do
      ...
      cond do
        ...
        true ->
          {:error, "Observation resourceType field is required"}
      end
    end
  ...
  describe "show/2" do
    ...
    test "Responds with observation info if the observation is found", %{
      conn: conn,
      sport: sport,
      testdata: testdata
    } do
    ...
    @tag integration: true
    test "Responds with a message indicating observation not found", %{conn: conn, sport: sport} do
      run_server(
        VanillaServer,
        fn port ->
          {:ok, channel} = GRPC.Stub.connect("localhost:#{port}")
  ...
  describe "update/2" do
    ...
    @tag integration: true
    test "Edits, and responds with the observation if attributes are valid", %{
      conn: conn,
      sport: sport,
      testdata: testdata
    } do
    ...
    @tag integration: true
    test "Returns an error and does not edit the observation if attributes are invalid", %{
      conn: conn,
      sport: sport,
      testdata: testdata
    } do
  ...
  describe "create/2" do
    ...
    @tag integration: true
    test "Creates, and responds with a newly created observation if attributes are valid", %{
      conn: conn,
      sport: sport
    } do
      run_server(
    ...
    @tag integration: true
    test "Returns an error and does not create the observation if attributes are invalid", %{
      conn: conn,
      sport: sport
    } do
      run_server(
  ...
  describe "delete/2" do
    setup [:temp_observation, :verify_on_exit!]

    @tag integration: true
    test "Deletes, and responds with :ok if observation was deleted", %{
      conn: conn,
      sport: sport,
      temp_data: testdata
    } do
  ...
  defp temp_observation(%{sport: sport}) do
    {:ok, reply} =
      run_server(
        VanillaServer,
        fn port ->
